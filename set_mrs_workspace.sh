...
if ! $NO_BUILD; then
  [ -z "$GITHUB_CI" ] && command catkin build -j3 mavros -c --mem-limit 75%
  [ ! -z "$GITHUB_CI" ] && command catkin build -j3 mavros --limit-status-rate 0.2 --summarize
  [ -z "$GITHUB_CI" ] && command catkin build -j3 mavlink_sitl_gazebo -c --mem-limit 75%
  [ ! -z "$GITHUB_CI" ] && command catkin build -j3 mavlink_sitl_gazebo --limit-status-rate 0.2 --summarize
  [ -z "$GITHUB_CI" ] && command catkin build -j3 mrs_gazebo_common_resources -c --mem-limit 75%
  [ ! -z "$GITHUB_CI" ] && command catkin build -j3 mrs_gazebo_common_resources --limit-status-rate 0.2 --summarize
  [ -z "$GITHUB_CI" ] && command catkin build -j3 -c --mem-limit 75%
  [ ! -z "$GITHUB_CI" ] && command catkin build -j3 --limit-status-rate 0.2 --summarize
fi
